...
class DataProto:
    ...
    '''
    batch: 一个 TensorDict 对象，用于存储 PyTorch 张量数据
    non_tensor_batch: 一个字典，用于存储非张量数据（通常是 NumPy 数组）
    meta_info: 一个字典，用于存储元信息（如配置参数等）
    '''
    batch: TensorDict = None
    non_tensor_batch: dict = field(default_factory=dict)
    meta_info: dict = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        '''
        batch 的 batch_size 必须是一维的（只支持 num_batch_dims=1）
        non_tensor_batch 中的所有值必须是 np.ndarray
        batch 和 non_tensor_batch 的第一维度必须相同
        '''
    ...
    def __getitem__(self, item):
        ...
        # 1. 单个索引 - 返回 DataProtoItem
        item = data_proto[5]

        # 2. 切片 - 返回 DataProto
        sliced = data_proto[10:20]
        sliced = data_proto[::2]

        # 3. 列表索引 - 返回 DataProto
        indices = [1, 5, 10, 15]
        selected = data_proto[indices]

        # 4. Tensor/ndarray 索引 - 返回 DataProto
        indices = torch.tensor([0, 2, 4])
        selected = data_proto[indices]

        # 5. 布尔索引 - 返回 DataProto
        mask = torch.tensor([True, False, True, False])
        selected = data_proto[mask]    
